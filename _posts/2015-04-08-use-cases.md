...
In this section, we highlight some use cases that you can easily implement and **enhance your user experience.**
...
Creating the same effect in your app requires you to integrate HOKO with your back-end and app. First you need to create an endpoint in your back-end API that will be called by the *magic* button. This endpoint will send an email to the user with a lazy smart link  (e.g. `http://app.hoko.link/lazy?uri=%2flogin%2Fa94f6b68ea32a4`) that will do the magic of opening your app and follow the respective authentication route. Read more about <a href="http://support.hokolinks.com/api/rest-creating-lazy-smartlinks/">lazy smart links</a>.

On the app side, all we need to do is map a `login/:auth_token` route that will validate the authentication token through the back-end, set the in-app token without ever needing the user's password and present the content to the user if everything went well.
...
HOKO's works perfectly as both reengagement and acquisition campaigns due to the nature of how smart links work (with and without the app installed).
